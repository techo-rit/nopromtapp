...
export const TemplateExecution: React.FC<TemplateExecutionProps> = ({
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-8 py-12">
        <button onClick={onBack} aria-label={`Back to ${stack.name} templates`} className="min-h-[44px] flex items-center gap-2 text-[#c9a962] hover:text-[#d4b872] mb-8 transition-colors rounded-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c9a962]/50 text-lg">
            <ArrowLeftIcon />
            Back to {stack.name}
        </button>
        <div className="text-center mb-10">
            <h1 className="text-[44px] md:text-[64px] lg:text-[80px] font-bold md:font-semibold tracking-[-0.02em] lg:tracking-[-0.03em] leading-[1.1] md:leading-[1.05] text-[#f5f5f5]" style={{fontFamily: 'var(--font-sans)'}}>{template.name}</h1>
            <p className="text-[17px] md:text-[19px] lg:text-[21px] font-medium tracking-normal md:tracking-[0.01em] lg:tracking-[0.015em] leading-[1.4] lg:leading-[1.33] text-[#a0a0a0] mt-3">Upload your image(s) or paste from clipboard.</p>
        </div>
        ...
        {isLoading && (
            <div className="flex flex-col items-center justify-center text-center p-16 bg-[#141414] rounded-2xl border border-[#2a2a2a]">
                <Spinner />
                <h2 className="text-[28px] md:text-[40px] font-semibold tracking-[0.005em] md:tracking-[-0.01em] leading-[1.2] md:leading-[1.1] text-[#f5f5f5] mt-6">Generating your images...</h2>
                <p className="text-[#a0a0a0] mt-2 text-lg">This usually takes about 20-30 seconds. Please wait.</p>
            </div>
        )}
        ...
        {error && !isLoading && (
             <div className="text-center p-8 bg-red-900/20 border border-red-800/50 rounded-2xl">
                 ...
                 <button onClick={handleRemix} className="min-h-[48px] mt-6 px-8 py-3 bg-red-600 text-white font-semibold rounded-xl hover:bg-red-700 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-red-600 text-lg">Try Again</button>
             </div>
        )}
            ...
            <div className="text-center">
                ...
                <div className={`grid grid-cols-1 ${generatedImages.length > 1 ? 'md:grid-cols-2' : ''} gap-6 max-w-5xl mx-auto mb-8`}>
                    {generatedImages.map((imageSrc, index) => (
                        <div key={index} className="relative group border border-[#2a2a2a] rounded-2xl shadow-lg overflow-hidden bg-[#141414]">
                            ...
                            <div className="absolute bottom-3 right-3 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                               <button onClick={() => handleDownload(imageSrc, 'png', index)} aria-label={`Download image ${index + 1} as PNG`} className="min-h-[44px] min-w-[44px] p-2 bg-[#141414]/90 backdrop-blur-sm rounded-full text-[#f5f5f5] hover:bg-[#1a1a1a] hover:text-[#c9a962] transition-colors shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c9a962]/50">
                                   <DownloadIcon/>
                               </button>
                               <button onClick={() => handleDownload(imageSrc, 'jpeg', index)} aria-label={`Download image ${index + 1} as JPG`} className="min-h-[44px] min-w-[44px] p-2 bg-[#141414]/90 backdrop-blur-sm rounded-full text-[#f5f5f5] hover:bg-[#1a1a1a] hover:text-[#c9a962] transition-colors shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c9a962]/50">
                                   JPG
                               </button>
                            </div>
                        </div>
                ...
                <div className="flex flex-wrap items-center justify-center gap-4">
                    <button onClick={handleRemixAgain} className="min-h-[56px] inline-flex items-center gap-2 px-8 py-4 bg-[#1a1a1a] text-[#f5f5f5] font-semibold rounded-xl hover:bg-[#2a2a2a] transition-colors border border-[#2a2a2a] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c9a962]/50 text-lg">
                      <RefreshIcon/>
                      Start Over
                    </button>
                    <button onClick={onBack} className="min-h-[56px] inline-flex items-center gap-2 px-8 py-4 bg-[#0a0a0a] text-[#a0a0a0] font-semibold rounded-xl hover:bg-[#1a1a1a] hover:text-[#f5f5f5] transition-colors border border-[#2a2a2a] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#c9a962]/50 text-lg">
