...
export const OutfitCard: React.FC<OutfitCardProps> = ({ outfit, onTryOn }) => {
  ...
  return (
    <div className="group relative rounded-2xl overflow-hidden bg-gradient-to-b from-[#141414] to-[#0d0d0d] border border-[#1e1e1e] hover:border-[#c9a962]/40 transition-all duration-300">
      {/* Garment Collage */}
        ...
        <div className={`grid h-full w-full ${garments.length >= 3 ? 'grid-cols-2 grid-rows-2' : garments.length === 2 ? 'grid-cols-2' : ''}`}>
          {garments.slice(0, 4).map((g, i) => (
            <div key={g.id} className={`relative overflow-hidden ${garments.length === 3 && i === 0 ? 'row-span-2' : ''}`}>
              ...
              <div className="absolute inset-0 border-[0.5px] border-[#1e1e1e]" />
            </div>
          ))}
        </div>

        {/* Match percentage badge */}
        <div className="absolute top-3 right-3 bg-[#0a0a0a]/80 backdrop-blur-sm border border-[#c9a962]/30 rounded-full px-2.5 py-1 flex items-center gap-1.5">
          <div className="w-1.5 h-1.5 rounded-full bg-[#c9a962]" />
          <span className="text-[11px] font-semibold text-[#c9a962] tracking-wide">{matchPct}%</span>
        </div>
      ...
      <div className="p-4 space-y-2.5">
        <h3 className="font-['Playfair_Display'] text-base text-white leading-tight truncate">
        ...
        {outfit.vibe_occasions && outfit.vibe_occasions.length > 0 && (
          <div className="flex flex-wrap gap-1.5 pt-1">
            {outfit.vibe_occasions.slice(0, 3).map((occ) => (
              <span
                key={occ}
                className="text-[10px] uppercase tracking-widest text-[#c9a962]/70 bg-[#c9a962]/8 border border-[#c9a962]/15 rounded-full px-2.5 py-0.5"
              >
                {occ}
              </span>
            ))}
        ...
        {onTryOn && (
          <button
            ...
            className="w-full mt-2 py-2.5 rounded-xl text-[12px] uppercase tracking-[0.15em] font-semibold
              ...
              hover:from-[#d4b872] hover:to-[#c9a962] active:scale-[0.98] transition-all duration-200"
          >
            Try This On
          </button>
        )}
